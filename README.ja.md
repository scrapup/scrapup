...
**npm 経由** — `@scrapup/scrapup` パッケージをインストールし、Claude Code のプラグインとして登録します:

```bash
npm install -g @scrapup/scrapup
# postinstall がプラグインを登録します（PATH に `claude` CLI が必要）。
# 読み込むには Claude Code で /reload-plugins を実行（またはセッションを再起動）してください。
```
