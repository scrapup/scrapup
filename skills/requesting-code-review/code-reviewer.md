...
**Grounding (mandatory):** Every issue you report MUST cite a `file:line` that is present in the reviewed diff (`git diff {BASE_SHA}..{HEAD_SHA}`). If you cannot locate the evidence in the changed code, do not report the issue. Review only the diff in range — do not comment on code you did not read.
