---
name: scrapup-inception
description: Coleta os scraps iniciais (documentos, mensagens, prompt, transcrições ou um conjunto de artefatos) e produz o brief.md — o artefato único e completo da fase de Inception — em camadas validadas (cadeia de lastro), com profundidade rasa, perguntando ao utilizador em vez de inferir lacunas, e fechando por uma revisão LCO criteriosa via agent reviewer-process-lco. Use quando o utilizador quiser iniciar uma feature/iniciativa a partir de material bruto, produzir o brief de Inception, coletar scraps, fundamentar o "porquê/o quê" antes de especificar, ou abrir o ciclo na fase de Inception.
user-invocable: true
metadata:
  obsidian_identifier: scrapup:scrapup-inception
---
...
forem vagos. Tom: `/scrapup:perfil-utilizador`.
...
- **Tom:** `/scrapup:perfil-utilizador`. **Estado:** `/scrapup:saga-session`.
