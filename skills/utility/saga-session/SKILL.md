...
user-invocable: true
metadata:
  obsidian_identifier: scrapup:saga-session
---
...
Quando usado em conjunto com a skill /scrapup:scrapup-forge ou planos SDD:
...
#### Integracao com mimic-loop

Durante execucao de TFs via /scrapup:scrapup-forge + /scrapup:mimic-loop:

- Projecto saga: `exec:{repo}:{TF-XX-YY}` (temporario, lifecycle da execucao)
- Cada iteracao subagent: `task_update` (status in_progress) + `comment_add` (breadcrumb) + `note_save` (handoff)
- Tipos de note usados pelo mimic-loop (`guardrail`, `metrics`, `metrics_summary`, `signal`, `context`, `blocker`, `progress`): ver a tabela "Tipos de note e quando usar" — fonte unica de definicao

Ver skill /scrapup:mimic-loop para detalhes do fluxo completo.
...
| `context` | Informacao que o proximo agente precisa para retomar. BASELINE_OVERRIDES | saga-session, scrapup-forge |
| `technical` | Detalhes tecnicos: root cause, mecanismo, comportamento. Findings de self-review. Baseline de homogeneidade. Grafo source-to-test | saga-session, scrapup-forge, multi-spec-review (via saga_writes de reviewer-homogeneity e reviewer-testing) |
| `blocker` | Impedimento ativo que precisa de resolucao | saga-session |
| `progress` | Resumo de sessao — o que foi feito, o que falta. Resumo de consolidacao e execucao | saga-session, scrapup-forge |
| `override` | Decisao do utilizador para pular validacao (CI override, 10 ciclos esgotados, --no-verify autorizado) | scrapup-forge |
| `metrics` | Dados quantitativos pos-execucao (tempo total, ciclos, taxa de sucesso, issues Sonar). Metricas de performance dos agents reviewer-* | scrapup-forge, multi-spec-review |
| `lesson` | Licao aprendida qualitativa (padroes de falha, o que funcionou, sugestoes recorrentes do self-review) | scrapup-forge |
| `guardrail` | Instrucoes acumulativas derivadas de falhas de iteracao. No MCP, gravar como `technical` com tag `guardrail`; tipo formal no store local `.mimic/` | mimic-loop |
| `signal` | Classificacao do resultado de uma iteracao (`COMPLETE`/`ROTATE`/`GUTTER`/`DEFER`) | mimic-loop |
| `metrics_summary` | Resumo compacto de metricas de iteracoes anteriores (consolidado de varios `metrics`) | mimic-loop |
...
Fonte canonica de todos os projetos que vivem no SQLite do saga. Skills consumidoras (scrapup-forge, multi-spec-review) referenciam esta secao para convencoes — nao definem regras proprias de persistencia. Agents reviewer-* operam em readonly e declaram escritas via `saga_writes` que o multi-spec-review executa.
...
| Configuracao de testes | `test-config:{repo}` | scrapup-forge (secao 4.1) | Scripts de teste, lint e build por repositorio. Atualizado quando package.json muda. Cache para subagents |
...
| Execucao TF/US | `exec:{repo}:{TF-XX-YY\|US-XX}` | scrapup-forge §4.2 | `project_update` → archived apos PR Open |
...
| `test-config:` | Configuracao de testes por repo (scripts, hash) | Persistente |
...
| `exec:` | Execucao de tarefa/US pelo scrapup-forge | Temporario |
...
1. **Persistentes** (`homogeneity:`, `contradiction-precedents:`, `architecture-decisions`, `test-config:`, `test-graph:`, `scrapup`): NUNCA arquivar, NUNCA deletar notes. Atualizar via upsert (`note_save` com `id` existente)
...
| Verificar BASELINE_OVERRIDES ativo | `note_search` tipo `context` no projeto `exec:*` ativo |
| Decisoes de arquitetura por dominio | `note_search` no projeto `architecture-decisions` com keyword `[nestjs]`, `[fastify]`, etc. |
| Obter scripts de teste do repo | `project_list` → filtrar `test-config:{repo}` → `note_list` |
