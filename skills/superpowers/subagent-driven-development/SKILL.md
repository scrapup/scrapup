...
metadata:
  obsidian_identifier: scrapup:subagent-driven-development
---
...
- **Readiness:** antes de despachar o primeiro implementer, classificar o projeto via /scrapup:baseline-assessment; não implementar se o gate reprovar.
...
- **Fronteira:** para TF/US do fluxo SDD, preferir /scrapup:scrapup-forge (que orquestra /scrapup:mimic-loop). Esta skill é para **planos escritos genéricos** executados na sessão atual.
...
digraph process {
    ...
    "Classify readiness (baseline-assessment), read plan, extract all tasks, persist in saga (+ TodoWrite mirror)" [shape=box];
    ...
    "Classify readiness (baseline-assessment), read plan, extract all tasks, persist in saga (+ TodoWrite mirror)" -> "Dispatch implementer subagent (./implementer-prompt.md)";
...
- **/scrapup:baseline-assessment** - REQUIRED: readiness gate before dispatching the first implementer
